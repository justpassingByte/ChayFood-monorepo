...
export default function AdminSidebar() {
  ...
  return (
    <aside
      className={`bg-[#081C15] border-r border-[#0F3528] text-slate-200 transition-all duration-300 flex flex-col h-screen select-none z-20 flex-shrink-0 ${
        isSidebarCollapsed ? 'w-[72px]' : 'w-64'
      }`}
    >
      {/* Brand Header */}
      <div className="h-16 px-3.5 border-b border-[#0F3528] flex items-center justify-between">
        {!isSidebarCollapsed ? (
          <Link href="/admin" className="flex items-center space-x-3 group min-w-0">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-emerald-500 to-teal-400 flex items-center justify-center text-slate-950 shadow-md shadow-emerald-950/40 flex-shrink-0">
              <SparklesIcon className="w-4 h-4 text-slate-950" />
            </div>
            <div className="flex flex-col truncate">
              <span className="text-xs font-bold tracking-wider text-white uppercase group-hover:text-emerald-300 transition-colors truncate">
                ChayFood Admin
              </span>
          ...
          <Link href="/admin" className="mx-auto" title="ChayFood Admin Portal">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-emerald-500 to-teal-400 flex items-center justify-center text-slate-950 shadow-md shadow-emerald-950/40">
              <SparklesIcon className="w-4 h-4 text-slate-950" />
            </div>
          </Link>
        )}
        ...
        <button
          ...
          className={`p-1.5 rounded-lg text-emerald-400/80 hover:text-white hover:bg-emerald-900/40 transition-colors ${
      ...
      <nav className="flex-1 overflow-y-auto px-2.5 py-3.5 space-y-1.5 custom-scrollbar">
        {navigation.map((item) => {
          const isActive =
            item.href === '/admin'
              ? pathname === '/admin'
      ...
      {isSidebarCollapsed && (
        <div className="p-2 border-t border-[#0F3528] flex justify-center">
          <button
            onClick={toggleSidebar}
            className="p-2 rounded-xl text-emerald-400/80 hover:text-white hover:bg-emerald-900/40 transition-colors"
      ...
      <div className="p-2.5 border-t border-[#0F3528] bg-[#05140F]">
        {!isSidebarCollapsed ? (
          <div className="flex items-center space-x-2.5 px-2 py-1.5 mb-1.5 rounded-xl bg-emerald-950/30 border border-emerald-900/40">
            <div className="w-7 h-7 rounded-lg bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 flex items-center justify-center font-bold text-xs flex-shrink-0">
              {user?.name?.charAt(0)?.toUpperCase() || 'A'}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-slate-100 truncate whitespace-nowrap">
        ...
        <button
          ...
          className={`w-full flex items-center py-2 rounded-xl text-xs font-semibold text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 transition-colors border border-transparent hover:border-rose-500/20 ${
